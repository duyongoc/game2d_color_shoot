...
public class TweenPosition : MonoBehaviour
{
    ...
    public void Play()
    {
        ...
        else
        {
            ...
            {
                transform.localPosition = From;
                transform.AsRectTransform().DOAnchorPos(To * Multiple, Duration).SetDelay(Delay).SetLoops(LoopTimes, loopType).SetEase(ease)
                    .OnComplete(
                        () =>
                        {
                            OnFinished?.Invoke();
            ...
            {
                transform.position = From;
                transform.DOMove(To * Multiple, Duration).SetDelay(Delay).SetLoops(LoopTimes, loopType).OnComplete(
                    () =>
                    {
                        OnFinished?.Invoke();
    ...
    public void Rewind()
    {
        ...
        {
            transform.localPosition = To;
            transform.AsRectTransform().DOAnchorPos(From, Duration).SetLoops(LoopTimes, loopType).OnComplete(() =>
            {
                OnFinished?.Invoke();
                transform.localPosition = From;
            })
